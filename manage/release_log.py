...
mysql_port = 3306
mysql_charset = "utf8"
bak_path = "/data/DBbackup/log_bakup/"
...
def main(host):
	try:
		...
		cur = conn.cursor()
		cur.execute("show databases like 'nx_log_%';")
		data = cur.fetchall()
		if len(data) <= 0:
			print('{0}{1} {2}该实例上没有log库,请确认该实例是否为日志库!{3}'.format(st.color['FG_YELLOW'],host,st.color['FG_RED'],st.color['END']))
			sys.exit(0)
		for dbs in data:
			for db in dbs:
		...
		if len(delete_db_list) <= 0:
			print('{0} {1} 该数据库实例上没有被合掉的log库,请再次确认操作的数据库实例对象!{2}'.format(st.color['FG_RED'],host,st.color['END']))
			sys.exit(0)
